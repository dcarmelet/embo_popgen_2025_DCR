...
import csv
open=open("ResultsPrac2_1.csv", "w")
...
def repeat_simulations(mut, sample_sizes, demography, length, reco, num_simulations, seed=None):
    ...
    for i in tqdm(range(num_simulations), desc="Running simulations"):
        ...
        demography.add_population(name="N1", initial_size=60000)
        demography.add_population(name="N2", initial_size=2000)
        ...
        demography.add_population_split(time=Time, derived=["N1", "N2"], ancestral="NA")
        ...
        sample_sets=[mutated_ts.samples(pop_id["N1"]), mutated_ts.samples(pop_id["N2"])]
