...
Plot the posterior probability, assuming a uniform prior (alpha = beta = 1).
Then calculate the maximum a posteriori value, 95% credible intervals ***(0.16, 0.25)***, and notable quantiles.
...
k<-40
xs<-seq(0,1,0.001)
Y<-dbeta(xs,k+1,n-k+1)
plot(xs,Y,type="l",col="red")

#the mode of beta distribution is (alpha - 1)/(alpha-beta - 2)
xs[Y==max(Y)]

#the 95%  credibility interval is between 2.5% and 97.5%
qbeta(c(.025,.975),k+1,n-k+1)

qbeta(c(.25,.75),k+1,n-k+1)
...
The genome-wide distribution of allele frequencies for human populations as a particular shape.
This is called a site frequency spectrum (SFS) or allele frequency spectrum (AFS).
...
Also, we don't know *a priori* whether the allele we are interested in is the minor allele.
...
Calculate the Bayes factor for a model with $p<=0.5$ vs a model with $p>0.5$.
Note that these models are equally probable a priori.
