evolve  <- function(Propability=.01,numberGen=100,Ne=100) {
  
  Pop<-rep(0,Ne)
  
  for (i in 1:numberGen) {
    
  }
  
  
  Mutationrate<-0
  Mutrate<-Propability*Popsize
  
}

#Consider reverse ? 

next.gen2 <-function(coin,Ne,f){
...
revers<-
  ...
  evolve  <- function(f,ngen,Ne) {
    coin<-rep(0,Ne)
    for (i in 1:ngen) {
      coin<-next.gen2(coin,Ne,f)
      print(mean(coin))
    }
  }
...
tmrca<-function(Ne,l){
  ...
  for(i in 1:l){
    g=1
    while (sample(1:Ne,1)!=sample(1:Ne,1)) {
      g=g+1
    }
    tmrca[i]<-g
  }  
...
library(reshape2)
dfm<-melt(df)
ggplot(dfm)+geom_density(aes(x=value,col=variable))+theme_bw()
...
### This exe is on ther server, we find solution locally
